...
<?php
 function validerRecap($connexion) {
    ...
    if ($result0 && mysqli_num_rows($result0) > 0) {
        ...
        $ajouterPrixClient= "UPDATE devis SET prixClient='$prixClient' WHERE idDev='$idDev'";
        ...
        $ajouterPrixMois= "UPDATE devis SET prixMois='$prixMois' WHERE idDev='$idDev'";
        $resultAjouterPrixMois = mysqli_query($connexion, $ajouterPrixMois);
        if (!$resultAjouterPrixMois) {
            die("Erreur lors de la mise à jour du prix mensuel : " . mysqli_error($connexion));
        }   
