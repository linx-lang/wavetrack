...
<?php
    ...
    function enleverAlerte($connexion, $idA) {
    $idA = intval($idA);
    $sql = "UPDATE alerte SET resolu = 1 WHERE idA = " . $idA;
    ...
    if (isset($_GET['id'])) {
        // Vérifier que la salle demandée appartient bien au propriétaire connecté
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"])) {
            $action = $_POST["action"];
            if ($_POST["action"] === "enleverAlerte") {
                $idA = (int)$_POST["idA"];
                if ($idA > 0) {
                    mysqli_query($connexion, "UPDATE alerte SET resolu = 1 WHERE idA = $idA");
                }
                header("Location: Salle.php?id=" .$id_salle. "&lundi=" . urlencode($lundi));
                exit;
            }
        
            if ($action === "resoudreAlerte" && isset($_POST["id"])) {
                enleverAlerte($connexion, $_POST["id"]);
                header("Location: Salle.php?id=" . $id_salle . "&lundi=" . urlencode($lundi));
                exit;
            }
...
<body class="bg-light">
    ...
    <nav class="navbar">
        ...
        <ul class="nav-links">
            ...
            <li><a href="profil.php"><img src="img/user.png" alt="Profile" class="user-img"></a></li>
        </ul>
    </nav>
                ...
                <div class="conteneur">
                            <div class="planning-header">
                                ...
                                <div class="planning-nav">
                                    <a href="Salle.php?id=<?= $id_salle ?>&lundi=<?= urlencode($lundiAvant) ?>" class="boutton">« Semaine précédente</a>
                                    <span>
                    ...
                    <div class="tab-pane fade" id="alertes">
                        ...
                        <div class="list-group">
                            ...
                            <div class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <form method="POST" action="Salle.php?id=<?= $id_salle ?>">
                                    <input type="hidden" name="action" value="resoudreAlerte"><!-- pour différencier des autres actions de formulaire -->
                                ...
                                <div class="card shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <p><strong>Compteur :</strong>
                                            <span id="nbVestiaires"><?= $nbVestiaires; ?> personnes</span>
                                        </p>
                                    </div>
                                </div>
                            </div>
